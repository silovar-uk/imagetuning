import type { LayerDirection, LayerKind } from '../document/order';
import type {
  AppDocument,
  CanvasSettings,
  CommentObject,
...
export type EditorAction =
  ...
  | { type: 'ADD_SHAPE'; shape: ShapeObject }
  | { type: 'UPDATE_SHAPE'; shapeId: string; patch: Partial<ShapeObject> }
  ...
  | { type: 'SET_TOOL_OPTIONS'; patch: Partial<ToolOptions> }
  | { type: 'SET_MODAL'; modal: ModalId }
  | { type: 'PATCH_SETTINGS'; patch: Partial<EditorSettings> }
  | { type: 'UPDATE_CANVAS'; patch: Partial<CanvasSettings> }
  | { type: 'MOVE_LAYER'; kind: LayerKind; id: string; direction: LayerDirection }
